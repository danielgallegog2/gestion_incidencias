...
import { User as UserEntity } from '../entities/User'; // Assuming UserEntity is the TypeORM entity
...
export class UserAdapter implements UserPort {
    ...
    private toEntity(usuario: Omit<UserDomain, 'id'>): UserEntity {
        ...
        userEntity.status = usuario.status || 1; // Default status to 1 if not provided
        return userEntity;
    }

    async createUser(usuario: Omit<UserDomain, 'id'| 'create_en'>): Promise<number> {
        try {
            const newUser = this.toEntity(usuario);
            const savedUser = await this.UserRepository.save(newUser);
            return savedUser.id_usuarios; // Assuming id_usuarios is the primary key in UserEntity
        } catch (error) {
    ...
    async updateUser(id: number, user: Partial<UserDomain>): Promise<boolean> {
        try {
            const existingUser = await this.UserRepository.findOne({where: { id_usuarios: id }});
            if (!existingUser) {
            ...
            Object.assign(existingUser,{
                ...
                status: 1 // Assuming status is always set to 1 for active users,
            });
    ...
    async deleteUser(id: number): Promise<boolean> {
        try {
            ...
            // Update only the fields that are provided
            Object.assign(existingUser,{
                status: 0 // Assuming status 0 means the user is deleted or inactive
            });
            await this.UserRepository.save(existingUser);
            return true;
        } catch (error) {
